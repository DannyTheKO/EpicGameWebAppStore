...
import { GetAllgame, AddGame, DeleteGame,GetAllPublisher,GetAllGenre ,UpdateGame} from "./API";  // Import các API
import "./table.css";
...
function Game() {
  ...
  const fetchGame = async () => {
    ...
    try {
      ...
      const [res, genre, publisher] = await Promise.all([GetAllgame(), GetAllGenre(), GetAllPublisher()]);
...
// };
  ...
  const validateGameRecord = () => {
    const { title, author, price, rating, release, description, publisherId, genreId } = gameRecord;
  
    // Kiểm tra Publisher và Genre
    if (!publisherId) {
      Modal.error({
        title: 'Error',
        content: 'Please fill in all required fields with valid information and select a Publisher .',
      });
      return false;
    }
    if (!genreId) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record) => (
        <Space size="middle">
          <Button onClick={() => openModal()} type="primary">Add</Button>
...
>
  ...
  <Select
    ...
    onChange={(value) => setGameRecord({ ...gameRecord, publisherId: value })}  // Cập nhật publisherId khi chọn publisher
    style={{ width: "100%", height: "52px" }}
  >
    {datapublisher.map((publisher) => (
      <Option key={publisher.publisherId} value={publisher.publisherId}>  {/* Sử dụng publisherId làm value */}
...
<Input
  ...
  onChange={(e) => {
    ...
    if (parsedValue === "" || (!isNaN(parsedValue) && parsedValue >= 0 && parsedValue <= 10)) {
